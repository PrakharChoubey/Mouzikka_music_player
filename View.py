...
class View:
    def __init__(self, top=None):
        ...
        self.nextButton.place(relx=0.785, rely=0.266, height=34, width=34)
        self.nextButton.configure(activebackground="#ececec")
        self.nextButton.configure(activeforeground="#000000")
        self.nextButton.configure(background="#fff")
        ...
        self._img66 = tk.PhotoImage(file="./icons/next.png")
        self.nextButton.configure(image=self._img66)
        self.nextButton.configure(pady="0")
        self.nextButton.configure(text='''Button''')
        ...
        self.song_seek = ttk.Scale(top)
        self.song_seek.place(relx=0.393, rely=0.228, relwidth=0.495
                             , relheight=0.0, height=17, bordermode='ignore')
        self.song_seek.configure(takefocus="")
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                messagebox.showinfo("Success", "Connected to the DB")
        ...
        self.song_seek.config(state='disabled')
        ...
        self.top.title("MOUZIKKA..")
        self.top.protocol("WM_DELETE_WINDOW", self.closewindow)
        self.previousButton.config(command=self.prev_song)
        self.nextButton.config(command=self.load_next_song)
        self.isPaused = False
    ...
    def move_seek(self, val):
        val2 = int(float(val))
        if val2 - self.music_pos > 500:
            self.music_pos = val2
            self.my_player.move_seek(val2)
        elif val2 - self.music_pos < (-500):
            self.music_pos = val2
            self.my_player.move_seek(val2)

    def show_seek(self):
        if self.song_seek.get() < 100:
            self.music_pos = int(self.my_player.music_pos())
        else:
            self.music_pos += 1000
        self.song_seek.set(self.music_pos)

    def add_song(self):
        songs = self.my_player.add_songs()
        if songs is None:
            return
        for song in songs:
    ...
    def remove_song(self):
        ...
        try:
            ...
            if self.isPlaying is True :
                if self.playList.get(self.sel_song_index_tuple[0])==self.song_name:
                    self.stop_song()
    ...
    def show_song_details(self):
        ...
        self.song_seek.config(state='enabled', from_=0,
                              to=self.song_length * 1000, command=self.move_seek)
        ...
        st = self.song_name[0:ext_index]
        if len(st) > 14:
            st = st[0:14] + "....."
        self.songName.configure(text=st)

    def play_song(self):
        self.music_pos = 0
        self.song_seek.set(0)
        ...
        try:
            ...
            self.pauseButton.configure(image=self._img1)
            self.isPaused = False
            # self.song_seek.set(self.song_length)
    ...
    def show_timer(self, total_sec):
        ...
        while curr_sec <= total_sec:
            ...
            if curr_sec != (self.music_pos // 1000)+ 1:
                self.songProgress.step(((self.music_pos // 1000)+ 2) - curr_sec)
                curr_sec = (self.music_pos // 1000)+2
            else:
    ...
    def pause_song(self):
        if self.isPlaying:
            if self.isPaused:
                ...
                self.pauseButton.configure(image=self._img2)
                self.isPaused = True
    ...
    def remove_song_from_favourites(self):
        sel_song_index_tuple = self.playList.curselection()
        try:
            if len(sel_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song to remove from favourites")
            song_name = self.playList.get(sel_song_index_tuple[0])
            result = self.my_player.remove_song_from_favourites(song_name)
            if result == 'Deleted':
                self.remove_song()
            messagebox.showinfo("Success!!!", "Successfully removed from favourites")
        except(NoSongSelectedError)as ex1:
            messagebox.showerror("Error!", ex1)
            print(ex1)
        except(DatabaseError)as ex2:
    ...
    def closewindow(self):
        if self.isThreadRunning:
            self.stop_song()
        print("Event fired")
        if messagebox.askyesno(title="App is closing!!!", message="Do you really want to quit?"):
